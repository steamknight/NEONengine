...
namespace mtl
{
    ...
    {
        return reinterpret_cast<T>(memAlloc(totalSize, static_cast<ULONG>(AllocationFlags)));
    }

    template <class T, MemF AllocationFlags>
    T* allocType()
    {
        return reinterpret_cast<T*>(memAlloc(sizeof(T), static_cast<ULONG>(AllocationFlags)));
    }
