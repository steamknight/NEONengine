...
namespace NEONengine
{
    ...
    static tUwCoordYX s_flags[6] =
        {
            {.uwY = 0, .uwX = 0}, 
            {.uwY = 0, .uwX = FLAG_WIDTH, }, // English
            {.uwY = 32, .uwX = 0},
            {.uwY = 32, .uwX = FLAG_WIDTH}, // Italian
            {.uwY = 64, .uwX = 0,},
            {.uwY = 64, .uwX = FLAG_WIDTH, }, // German
    };

    const Hotspot *pEnglish = NULL;
    const Hotspot *pItalian = NULL;
    ...
    void langSelectCreate(void)
    {
        ...
        Hotspot englishFlag = {
            .bounds = (tUwRect){.uwY = uwY, .uwX = uwX, .uwWidth = FLAG_WIDTH, .uwHeight = FLAG_HEIGHT},
