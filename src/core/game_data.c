...
GameDataResult gameDataLoad(const char *szFilePath)
{
    ...
    GameDataResult result = GDR_SUCCESS;
    ...
    GDL_VERIFY(ulMagic == *(ULONG *)dataFileMagic, GDR_NOT_NEON_FILE);
    GDL_VERIFY(ulVersion == GDL_SUPPORTED_VERSION, GDR_VERSION_NOT_SUPPORTED);

    g_pGameData = memAllocFastClear(sizeof(GameData));
    s_pGameDataCounts = memAllocFastClear(sizeof(GameDataCounts));

    // Load the chunks
    while (!fileIsEof(pFile))
    {
        ULONG ulChunkHeader;
        GameDataResult chunkResult = GDR_SUCCESS;
        ...
        GDL_VERIFY(chunkResult == GDR_SUCCESS, chunkResult);
    }
